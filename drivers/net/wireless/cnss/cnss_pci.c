...
/* avoid using module_param for msm-5.4 and later,
 * will implement this by debugfs after bring up.
 *
...
static int cnss_get_wlan_enable_gpio(struct cnss_wlan_gpio_info *gpio_info,
...
static int cnss_wlan_get_resources(struct platform_device *pdev)
{
	...
	if (!penv->wlan_en_vreg_support) {
		...
		if (ret) {
			pr_err("%s:Failed to config the WLAN_EN gpio\n",
			       __func__);
			goto err_gpio_wlan_en;
		}
	}
	vreg_info->state = true;
...
static void cnss_wlan_release_resources(void)
{
	...
	if (penv->wlan_bootstrap_gpio > 0)
		gpio_free(penv->wlan_bootstrap_gpio);
	cnss_put_wlan_enable_gpio();
	gpio_info->state = false;
	gpio_info->prop = false;
	cnss_wlan_vreg_set(vreg_info, false);
	if (vreg_info->soc_swreg)
		regulator_put(vreg_info->soc_swreg);
	if (vreg_info->ant_switch)
		regulator_put(vreg_info->ant_switch);
...
static void print_allocated_image_table(void)
{
	...
	tmp_buf = kmalloc(MAX_IMAGE_SIZE, GFP_KERNEL);
	if (!tmp_buf)
		return;
	...
	while (seg++ < penv->fw_seg_count) {
		...
		hex_dump_to_buffer(dump_addr, pseg_mem->size - sizeof(struct region_desc),
				   16, 1, tmp_buf, MAX_IMAGE_SIZE, false);
		pr_debug("%s: %s\n", __func__, tmp_buf);
		pseg_mem++;
	}
	...
	while (seg++ < penv->bdata_seg_count) {
		dump_addr = (u8 *)p_bdata_seg_mem->cpu_region +
			sizeof(struct region_desc);
		hex_dump_to_buffer(dump_addr, p_bdata_seg_mem->size - sizeof(struct region_desc),
				   16, 1, tmp_buf, MAX_IMAGE_SIZE, false);
		pr_debug("%s: %s\n", __func__, tmp_buf);
		p_bdata_seg_mem++;
	}
...
int cnss_msm_pcie_pm_control(enum msm_pcie_pm_opt pm_opt, u32 bus_num,
			     struct pci_dev *pdev, u32 options)
{
	return msm_pcie_pm_control(pm_opt, bus_num, pdev, NULL, options);
}

int cnss_pci_load_and_free_saved_state(struct pci_dev *dev,
			       ...
			       const struct pci_device_id *id)
{
	...
	cnss_configure_wlan_en_gpio(false);
	ret = cnss_wlan_vreg_set(vreg_info, false);
...
again:
	ret = cnss_wlan_vreg_set(vreg_info, true);
	if (ret) {
	...
	cnss_configure_wlan_en_gpio(true);
	...
	if (wdrv->probe) {
		...
		if (ret) {
			...
			cnss_msm_pcie_pm_control(MSM_PCIE_SUSPEND,
						 cnss_get_pci_dev_bus_number(pdev),
						 pdev, PM_OPTIONS);
			penv->pcie_link_state = false;
			cnss_configure_wlan_en_gpio(false);
...
cut_power:
	...
	cnss_configure_wlan_en_gpio(false);
	if (cnss_wlan_vreg_set(vreg_info, false))
...
cut_power:
	cnss_configure_wlan_en_gpio(false);
	if (cnss_wlan_vreg_set(vreg_info, false))
		pr_err("cnss: Failed to set WLAN false\n");
...
static int cnss_probe(struct platform_device *pdev)
{
	...
	penv->gpio_info.state = false;
	penv->gpio_info.init = false;
	penv->gpio_info.prop = false;
	penv->vreg_info.wlan_reg = NULL;
	penv->vreg_info.state = false;
	penv->pci_register_again = false;
	...
	ret = cnss_configure_wlan_en_gpio(true);
	if (ret) {
...
skip_ramdump:
	...
	if (penv->bus_scale_table)  {
		penv->bus_client =
			legacy_bus_register_client(penv->bus_scale_table);
...
int cnss_pci_request_bus_bandwidth(int bandwidth)
{
	...
	case CNSS_BUS_WIDTH_HIGH:
		ret = legacy_bus_client_update_request(penv->bus_client,
						       bandwidth);
		if (!ret) {
...
#ifdef CNSS_COMPLIE_ISSUE_FIX_LATER_IFNEEDED
	if (penv->bus_client)
		legacy_bus_client_update_request(penv->bus_client, option);
#endif
}
